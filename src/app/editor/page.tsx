...
export default async function EditorPage({ searchParams }: { searchParams: Promise<{ wf?: string; proj?: string }> }) {
  ...
  const project = proj
    ? await prisma.project.findUnique({ where: { id: proj }, select: { name: true, brandId: true, brand: { select: { name: true } } } })
    : null;
  ...
  return (
    <div className="h-screen overflow-hidden bg-bg flex flex-col">
      ...
      <VideoEditor assets={bin} workflowId={wf} projectId={proj} projectName={project?.name ?? undefined} brandId={project?.brandId ?? undefined} brandName={project?.brand?.name ?? undefined} />
    </div>
  );
}
